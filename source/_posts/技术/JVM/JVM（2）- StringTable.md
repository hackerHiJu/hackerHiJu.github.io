...
tags:
  - JVM
cover: /images/covers/JVM.jpg
thumbnail: /images/covers/JVM.jpg
published: true
abbrlink: 700835324
...
![image-20210916165312401](images/1.JVM（2）-StringTable.png)
...
![image-20210916165232659](images/2.JVM（2）-StringTable.png)
...
![image-20210916170155512](images/3.JVM（2）-StringTable.png)

![image-20210916170418495](images/4.JVM（2）-StringTable.png)
...
![image-20210924103436541](images/5.JVM（2）-StringTable.png)

- 先定义ab在串池当中
- 创建了两个string字符串，会先吧a和b字符串存入串池当中，当用StringBuilder进行拼接
- intern尝试将ab存入串池，但是ab已经存在于串池中，就不会存入，并且返回常量池中的引用

![image-20210924104110745](images/6.JVM（2）-StringTable.png)

- ab还没有存入串池中，s1尝试将ab放入串池中，串池中没有，就放入并且返回对象引用

![image-20210924104123965](images/7.JVM（2）-StringTable.png)
...
![image-20210924142800705](images/8.JVM（2）-StringTable.png)
...
![image-20210924143640973](images/9.JVM（2）-StringTable.png)
...
![image-20210924144237464](images/10.JVM（2）-StringTable.png)

将堆空间设置为10m，分配10000个字符串，就会触发Mirror GC

![image-20210924144351752](images/11.JVM（2）-StringTable.png)
