...
![在这里插入图片描述](images/1.JVM（1）-内存结构.png)

## 3. 常见JVM实现

不同的公司只要遵循虚拟机规范，都可以自己实现

![在这里插入图片描述](images/2.JVM（1）-内存结构.png)

## 4. 学习路线

![在这里插入图片描述](images/3.JVM（1）-内存结构.png)
...
![image-20210913144428655](images/4.JVM（1）-内存结构.png)
...
![image-20210910111558467](images/5.JVM（1）-内存结构.png)
...
![image-20210910112355067](images/6.JVM（1）-内存结构.png)
...
![image-20210910141553423](images/7.JVM（1）-内存结构.png)
...
![image-20210910141903711](images/8.JVM（1）-内存结构.png)
...
![image-20210910144500045](images/9.JVM（1）-内存结构.png)

![image-20210910144534701](images/10.JVM（1）-内存结构.png)

![image-20210910145341619](images/11.JVM（1）-内存结构.png)

![image-20210910145720366](images/12.JVM（1）-内存结构.png)

案例2：程序运行很长时间都没有得到自己想要的结果（线程死锁）

![image-20210910150600896](images/13.JVM（1）-内存结构.png)

![image-20210910150622803](images/14.JVM（1）-内存结构.png)

![image-20210910150837983](images/15.JVM（1）-内存结构.png)
...
![image-20210910151634764](images/16.JVM（1）-内存结构.png)
...
![image-20210910160109821](images/17.JVM（1）-内存结构.png)

查找出最大的对象

![image-20210910160143147](images/18.JVM（1）-内存结构.png)

可以发现其中最大的属性值是哪一个，可以发现每一个age属性都有1m多，总共创建了999个对象

![image-20210910160310422](images/19.JVM（1）-内存结构.png)

![image-20210910160426511](images/20.JVM（1）-内存结构.png)
...
![image-20210910161238546](images/21.JVM（1）-内存结构.png)

### 5.2 内存溢出

-XX: MaxMetaspaceSize=8m  调整方法区的大小（默认使用的系统内存，很难演示出元空间OOM）

![image-20210910163037193](images/22.JVM（1）-内存结构.png)
...
![image-20210929160545411](images/23.JVM（1）-内存结构.png)

直接内存，就是少拷贝了一份数据

![image-20210929160745231](images/24.JVM（1）-内存结构.png)
